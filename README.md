...
**Conditions:** single-sample inference (batch=1), single-threaded Python loop, CPU-only. PyTorch 2.x, Python 3.11, Apple M1 (ARM64). Each PyTorch call goes through the full Python → C++ dispatch path. Notation: arrows (`→`) show layer transitions; `×` denotes matrix dimensions (e.g., 128×784). When a number is prefixed with `≈` and followed by `×` (e.g., ≈258×), it denotes speedup.
...
> The 258× figure measures Python + PyTorch dispatch overhead on a 4-feature input, not PyTorch's raw compute. Once you batch (roughly 32+ samples) or run on GPU kernels, that overhead is amortized until the gap closes and PyTorch overtakes with its BLAS/GPU path (exact crossover depends on hardware/model).
